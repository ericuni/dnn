...
def main(argv):
	...
	'''
	The first way assigns a unique weight to each category.
	To do this, you must specify the category's vocabulary (values outside this specification will receive a weight of zero).
	Alternatively, you can define the vocabulary in a file (by calling `categorical_column_with_vocabulary_file`) or
	as a range of positive integers (by calling `categorical_column_with_identity`)
	'''
	...
	'''
	The second way, appropriate for an unspecified vocabulary, is to create a hashed column.
	It will create a fixed length list of weights, and automatically assign each input category to a weight.
	Due to the pseudo-randomness of the process, some weights may be shared between categories, while others will remain unused.
	'''
	...
	feature_columns = [
		...
		# This model adds two categorical colums that will adjust the price based on "make" and "body-style".
		body_style_column,
		make_column,
	]
